...
{
    SerializingHashMap<std::string, int> stringToInt0, stringToInt10(10), stringToInt1000(1000);
...
TEST_F(StringToIntTest, customHashFunctionWorksAsExpected)
{
    ...
    SerializingHashMap<std::string, int> stringToInt(0,
                                                     DefaultSerializingHashMapFunctions::keySerializerFunction<std::string>,
                                                     myHashFunction);

    ASSERT_TRUE(stringToInt.insert("hi0", 123));
    ASSERT_EQ(stringToInt.container[myHashFunction("hi0", stringToInt.container.size())]->key, "hi0");

    ASSERT_TRUE(stringToInt.insert("hi1", 123));
    ASSERT_EQ(stringToInt.container[myHashFunction("hi1", stringToInt.container.size())]->key, "hi1");
}